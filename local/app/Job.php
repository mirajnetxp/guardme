...
class Job extends Model
{
    ...
    {        
        ...
        */
        $distance_select_sub = sprintf(
                                   "           
>>>>>>> parent of e3c088e4... Point four done
...
// Added By miraj
		foreach ( $jobs_result as $key => $value ) {
			$applications                      = DB::table( 'job_applications' )
			                                       ->where( 'job_id', $jobs_result[ $key ]->id )
			                                       ->where( 'is_hired', 1 )
			                                       ->get();
			$industory                         = Businesscategory::find( $jobs_result[ $key ]->business_category_id );
			$schedules                         = SecurityJobsSchedule::find( $jobs_result[ $key ]->id );
			$jobs_result[ $key ]->applications = $applications;
			$jobs_result[ $key ]->industory    = $industory;
			$jobs_result[ $key ]->schedules    = $schedules;
		}
