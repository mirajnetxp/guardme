...
class User extends Authenticatable {
	...
	public function getReferrals() {
		if ( $this->id ) {
			$referrals  = Referral::where( 'to', $this->id )->get();
			$uReferrals = [];


			foreach ( $referrals as $key => $referral ) {
				$refaralUser                  = User::where( 'id', $referral->who )->first();
				$uReferrals [ $key ]['email'] = $refaralUser->email;

				//In case of emploryer
				if ( $refaralUser->admin == 0 ) {
					$numOfJobPost = DB::table( 'security_jobs' )->where( 'created_by', $refaralUser->id )->get();

					if ( count( $numOfJobPost ) > 0 && $referral->points == 0 ) {
				...
				//In case of freelancer
				if ( $refaralUser->admin == 2 ) {
					$hired = DB::table( 'job_applications' )
					           ->where( 'applied_by', $refaralUser->id )
					           ->where( 'is_hired', 1 )
					           ->get();
