            ...
            $("#create_job_form").on("submit", function (e) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var nextUrl = "{{ route('job.schedule.update', ":id") }}";
...
@section('content')
    ...
    <form id="create_job_form" method="POST" action="{{ route('api.update.job',['id'=>$job->id]) }}">


        <div class="form-group row">
            <label class="col-sm-3" for="job_title">Job Title</label>
            <div class="col-sm-9">
                <input type="text" name="title" class="form-control title" id="job_title" value="{{$job->title}}">
            ...
            <div class="col-sm-9">
                <select type="text" name="security_category" class="form-control" id="security_category">
                    ...
                    @foreach($all_security_categories as $sec_cat)
                        <option {{$sec_cat->id ==$job->security_category_id?'selected':''}} value="{{ $sec_cat->id }}">{{ $sec_cat->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="col-sm-9">
                <select type="text" name="business_category" class="form-control" id="business_category">
                    ...
                    @foreach($all_business_categories as $bus_cat)
                        <option {{$bus_cat->id ==$job->business_category_id?'selected':''}} value="{{ $bus_cat->id }}">{{ $bus_cat->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-9">
                <input id="line1" name="line1" class="form-control text-input validate[required]" type="text"
                       placeholder="Address line1" value="{{$job->address_line1}}">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-3">Address line2</label>
            <div class="col-sm-9">
                <input id="line2" name="line2" class="form-control text-input" type="text" placeholder="Address line2"
                       value="{{$job->address_line2}}">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-3">Address line3</label>
            <div class="col-sm-9">
                <input id="line3" name="line3" class="form-control text-input" type="text" placeholder="Address line3"
                       value="{{$job->address_line3}}">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-3">Town</label>
            <div class="col-sm-9">
                <input id="town" name="town" class="form-control text-input validate[required]" type="text"
                       placeholder="Town" value="{{$job->city_town}}">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-3">Country</label>
            <div class="col-sm-9">
                <input id="country" name="country" class="form-control text-input  validate[required]" type="text"
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-9">
                <input id="postcode" name="postcode" class="form-control text-input  validate[required]" type="text"
                       placeholder="Postalcode" value="{{$job->post_code}}">
